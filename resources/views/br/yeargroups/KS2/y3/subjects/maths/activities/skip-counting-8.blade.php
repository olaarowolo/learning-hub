...
<head>
  ...
  <title>Skip-counting by 8 - Year 3 Maths</title>
...
<body>
  <header class="year-header">
    <nav aria-label="breadcrumb" class="breadcrumb-nav">
      ...
      <span>Skip-counting by 8</span>
    </nav>
    <h1>Skip-counting by 8</h1>
  ...
  <div class="container">
    ...
    <main>
      ...
      <div class="lesson-note">
        <h3>Understanding Skip-Counting by 8</h3>
        <p>Skip-counting by 8 helps you recognise patterns and builds a foundation for multiplication and division by 8.</p>
        <ul>
          <li>Example: 8, 16, 24, 32, 40 …</li>
          <li>Notice that each number increases by 8.</li>
          <li>Practice using objects in groups of 8 to visualise patterns.</li>
        </ul>
        <p><strong>Tip:</strong> Count aloud and write your own sequences. This helps independent learning.</p>
      </div>
    </main>
    ...
    <main>
      ...
      <p>Practice skip-counting by 8 to strengthen number sense and prepare for multiplication facts.</p>

      <h3>Instructions</h3>
      <ul>
        <li>Count aloud in steps of 8 up to 100 or beyond.</li>
        <li>Fill in missing numbers in sequences below.</li>
        <li>Try counting objects in sets of 8 in real life.</li>
      </ul>
    </main>
  ...
  <script>
    const skipCounting8Activities = [
      {seq:[8,16,24,32,40], missing:[16,24,32]},
      {seq:[48,56,64,72,80], missing:[56,64,72]},
      {seq:[88,96,104,112,120], missing:[96,104,112]},
      {seq:[7,15,23,31,39], missing:[15,23,31]}, // fun variation
      {seq:[0,8,16,24,32], missing:[8,16,24]}
    ];

    const container = document.getElementById('interactive-skip-counting');

    skipCounting8Activities.forEach((act, idx) => {
      const div = document.createElement('div');
      div.className = 'interactive-box';
      div.innerHTML = `<strong>Activity ${idx+1}:</strong><br>Complete the sequence: ${act.seq.map((n,i)=> act.missing.includes(n)? `<input type="text" id="sc8-${idx}-${i}">`: n).join(', ')}
      <button onclick="checkSkip8(${idx})">Check Answers</button>
      <p id="sc8-${idx}-feedback"></p>`;
      container.appendChild(div);
    });

    function checkSkip8(idx){
      const act = skipCounting8Activities[idx];
      let correct = true;
      act.missing.forEach((val) => {
        const input = document.getElementById(`sc8-${idx}-${act.seq.indexOf(val)}`);
        if(parseInt(input.value) !== val) correct = false;
      });
      const feedback = document.getElementById(`sc8-${idx}-feedback`);
