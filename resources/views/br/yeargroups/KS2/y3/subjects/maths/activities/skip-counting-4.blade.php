...
<head>
  ...
  <title>Skip-counting by 4 - Year 3 Maths</title>
...
<body>
  <header class="year-header">
    <nav aria-label="breadcrumb" class="breadcrumb-nav">
      ...
      <span>Skip-counting by 4</span>
    </nav>
    <h1>Skip-counting by 4</h1>
  ...
  <div class="container">
    ...
    <main>
      ...
      <div class="lesson-note">
        <h3>Understanding Skip-Counting by 4</h3>
        <p>Skip-counting by 4 is counting forward in steps of 4. It helps you recognise patterns in numbers and is a foundation for multiplication by 4.</p>
        <ul>
          <li>Example: 4, 8, 12, 16, 20 …</li>
          <li>Notice the difference between consecutive numbers is always 4.</li>
          <li>Try counting objects in groups of 4 to see this in real life.</li>
        </ul>
        <p><strong>Tip:</strong> Practice aloud and try writing your own sequences. This helps independent learning and memory.</p>
      </div>
    </main>
    ...
    <main>
      ...
      <p>Practice skip-counting by 4. This strengthens number sense and prepares you for multiplication and division facts of 4.</p>

      <h3>Instructions</h3>
      <ul>
        <li>Count aloud in steps of 4 up to 100.</li>
        <li>Fill in missing numbers on the sequences below.</li>
        <li>Try real-life examples: count chairs in rows of 4, or sets of 4 pencils.</li>
      </ul>
    </main>
  ...
  <script>
    const skipCounting4Activities = [
      {seq:[4,8,12,16,20], missing:[8,12,16]},
      {seq:[24,28,32,36,40], missing:[28,32,36]},
      {seq:[44,48,52,56,60], missing:[48,52,56]},
      {seq:[64,68,72,76,80], missing:[68,72,76]},
      {seq:[1,5,9,13,17], missing:[5,9,13]} // fun variation starting from 1
    ];

    const container = document.getElementById('interactive-skip-counting');

    skipCounting4Activities.forEach((act, idx) => {
      const div = document.createElement('div');
      div.className = 'interactive-box';
      div.innerHTML = `<strong>Activity ${idx+1}:</strong><br>Complete the sequence: ${act.seq.map((n,i)=> act.missing.includes(n)? `<input type="text" id="sc4-${idx}-${i}">`: n).join(', ')}
      <button onclick="checkSkip4(${idx})">Check Answers</button>
      <p id="sc4-${idx}-feedback"></p>`;
      container.appendChild(div);
    });

    function checkSkip4(idx){
      const act = skipCounting4Activities[idx];
      let correct = true;
      act.missing.forEach((val) => {
        const input = document.getElementById(`sc4-${idx}-${act.seq.indexOf(val)}`);
        if(parseInt(input.value) !== val) correct = false;
      });
      const feedback = document.getElementById(`sc4-${idx}-feedback`);
