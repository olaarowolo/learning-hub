...
<head>
  ...
  <title>Skip-counting by 100 - Year 3 Maths</title>
...
<meta name="msapplication-square310x310logo" content="/favicons/ms-icon-310x310.png">
  <link rel="preconnect" href="https://fonts.googleapis.com" /><link href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/5.15.4/css/all.min.css" rel="stylesheet"><link rel="icon" type="image/x-icon" href="/favicons/favicon.ico">
    ...
    * { margin: 0; padding: 0; box-sizing: border-box; }
    ...
    .interactive-box input { width: 80px; padding: 5px; margin: 0 5px; text-align: center; }
...
<body>
  <header class="year-header">
    <nav aria-label="breadcrumb" class="breadcrumb-nav">
      ...
      <span>Skip-counting by 100</span>
    </nav>
    <h1>Skip-counting by 100</h1>
  ...
  <div class="container">
    ...
    <main>
      ...
      <div class="lesson-note">
        <h3>Understanding Skip-Counting by 100</h3>
        <p>Skip-counting by 100 helps you understand hundreds, place value, and prepares you for addition and subtraction with large numbers.</p>
        <ul>
          <li>Example: 100, 200, 300, 400, 500 …</li>
          <li>Notice each number increases by 100.</li>
          <li>Try grouping objects into sets of 100 to visualise patterns.</li>
        </ul>
        <p><strong>Tip:</strong> Write sequences and count aloud. This encourages independent learning and number sense.</p>
      </div>
    </main>
    ...
    <main>
      ...
      <p>Practice skip-counting by 100 to reinforce understanding of hundreds and number patterns.</p>

      <h3>Instructions</h3>
      <ul>
        <li>Count aloud in steps of 100 up to 1000 or beyond.</li>
        <li>Fill in the missing numbers in sequences below.</li>
        <li>Use real-life objects, like counting pages in a book or coins in bundles of 100.</li>
      </ul>
    </main>
    ...
    <main>
      ...
      <div id="interactive-skip-counting"></div>
    </main>
  </div>
  ...
  <script>
    const skipCounting100Activities = [
      {seq:[100,200,300,400,500], missing:[200,300,400]},
      {seq:[600,700,800,900,1000], missing:[700,800,900]},
      {seq:[50,150,250,350,450], missing:[150,250,350]}, // variation starting from 50
      {seq:[0,100,200,300,400], missing:[100,200,300]},
      {seq:[120,220,320,420,520], missing:[220,320,420]}
    ];

    const container = document.getElementById('interactive-skip-counting');

    skipCounting100Activities.forEach((act, idx) => {
      const div = document.createElement('div');
      div.className = 'interactive-box';
      div.innerHTML = `<strong>Activity ${idx+1}:</strong><br>Complete the sequence: ${act.seq.map((n,i)=> act.missing.includes(n)? `<input type="text" id="sc100-${idx}-${i}">`: n).join(', ')}
      <button onclick="checkSkip100(${idx})">Check Answers</button>
      <p id="sc100-${idx}-feedback"></p>`;
      container.appendChild(div);
    });

    function checkSkip100(idx){
      const act = skipCounting100Activities[idx];
      let correct = true;
      act.missing.forEach((val) => {
        const input = document.getElementById(`sc100-${idx}-${act.seq.indexOf(val)}`);
        if(parseInt(input.value) !== val) correct = false;
      });
      const feedback = document.getElementById(`sc100-${idx}-feedback`);
