...
<head>
  ...
  <title>Skip-counting Stories - Year 3 Maths</title>
...
<body>
  <header class="year-header">
    <nav aria-label="breadcrumb" class="breadcrumb-nav">
      ...
      <span>Skip-counting Stories</span>
    </nav>
    <h1>Skip-counting Stories</h1>
  ...
  <div class="container">
    ...
    <main>
      ...
      <div class="lesson-note">
        <h3>Understanding Skip-Counting Through Stories</h3>
        <p>Using real-life stories helps you understand skip-counting and see patterns in numbers.</p>
        <ul>
          <li>Example: Sam collects 5 apples each day. How many apples after 1, 2, 3 days? (5, 10, 15…)</li>
          <li>Notice how each step increases by the same number in the story.</li>
          <li>Try making your own stories using objects or steps around you.</li>
        </ul>
        <p><strong>Tip:</strong> Write sequences, solve problems, and explain your thinking for independent learning.</p>
      </div>
    </main>
    ...
    <main>
      ...
      <p>Practice skip-counting using story problems. This helps connect numbers with real-world situations.</p>

      <h3>Instructions</h3>
      <ul>
        <li>Read each story carefully.</li>
        <li>Write down the sequence of numbers that follows the story.</li>
        <li>Check your answers using the interactive section below.</li>
      </ul>
    </main>
    ...
    <main>
      ...
      <div id="interactive-stories"></div>
    </main>
  </div>
  ...
  <script>
    const storyActivities = [
      {story:"Sam collects 5 apples each day.", start:5, step:5, length:5},
      {story:"A bus picks up 10 passengers every stop.", start:10, step:10, length:5},
      {story:"Lily plants 3 flowers every hour.", start:3, step:3, length:5},
      {story:"Tom saves 2 coins each day.", start:2, step:2, length:5},
      {story:"A shop sells 7 candies per customer.", start:7, step:7, length:5}
    ];

    const container = document.getElementById('interactive-stories');

    storyActivities.forEach((act, idx) => {
      let seq = [];
      for(let i=0;i<act.length;i++) seq.push(act.start + i*act.step);
      const missing = seq.slice(1, seq.length-1); // hide middle numbers

      const div = document.createElement('div');
      div.className = 'interactive-box';
      div.innerHTML = `<strong>Story ${idx+1}:</strong> ${act.story}<br>Complete the sequence: ${seq.map((n,i)=> missing.includes(n)? `<input type="text" id="story-${idx}-${i}">`: n).join(', ')}
      <button onclick="checkStory(${idx})">Check Answers</button>
      <p id="story-${idx}-feedback"></p>`;
      container.appendChild(div);
    });

    function checkStory(idx){
      const act = storyActivities[idx];
      let seq = [];
      for(let i=0;i<act.length;i++) seq.push(act.start + i*act.step);
      const missing = seq.slice(1, seq.length-1);
      let correct = true;
      missing.forEach((val)=>{
        const input = document.getElementById(`story-${idx}-${seq.indexOf(val)}`);
        if(parseInt(input.value) !== val) correct = false;
      });
      const feedback = document.getElementById(`story-${idx}-feedback`);
