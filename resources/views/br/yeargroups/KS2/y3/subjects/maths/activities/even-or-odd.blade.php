...
<title>Even or Odd - Year 3 Maths</title>
...
<link rel="stylesheet" href="/data/yeargroups/breadcrumb.css" /><link href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/5.15.4/css/all.min.css" rel="stylesheet"><link rel="icon" type="image/x-icon" href="/favicons/favicon.ico">

    <link rel="stylesheet" href="/footer.css"><style>
:root{--color-year-3:#4caf50;--color-year-3-light:#81c784;--theme-color:var(--color-year-3);--theme-color-light:var(--color-year-3-light);--bg-color:#f8f9fa;--text-dark:#343a40;--text-light:#6c757d;--card-bg:#fff;--border-color:#dee2e6;--card-shadow:0 4px 15px rgba(0,0,0,0.06);}
...
.interactive-box{background-color:#e0f7fa;border-left:5px solid #4caf50;padding:1rem;border-radius:6px;text-align:center;}
.interactive-box input[type=text]{margin-right:0.5rem;width:80px;text-align:center;}
...
<h3>Understanding Even and Odd Numbers</h3>
<ul>
<li>Even numbers are divisible by 2 with no remainder (0,2,4...).</li>
<li>Odd numbers leave a remainder of 1 when divided by 2 (1,3,5...).</li>
<li>Practice recognising patterns independently to strengthen understanding.</li>
</ul>
</div>
</main>
...
<main>
    ...
    <p>Determine whether each number is even or odd.</p>
    <h3>Instructions</h3>
    <ul>
    <li>Type "Even" or "Odd" for each number below.</li>
    <li>Check your answers using the button provided.</li>
    </ul>
    </main>

<main>
<h2>Interactive Exercise</h2>
<div class="interactive-box-container" id="interactive-even-odd"></div>
</main>
</div>
...
const container=document.getElementById('interactive-even-odd');
numbers.forEach((num,idx)=>{
  const div=document.createElement('div');
  div.className='interactive-box';
  div.innerHTML=`${num}: <input type="text" id="eo-${idx}" placeholder="Even/Odd"> <button onclick="checkEO(${idx})">Check</button> <span id="eo-feedback-${idx}"></span>`;
  container.appendChild(div);
});
function checkEO(idx){
  const num=numbers[idx];
  const input=document.getElementById(`eo-${idx}`);
  const feedback=document.getElementById(`eo-feedback-${idx}`);
  const answer=num%2===0?'Even':'Odd';
  if(input.value.trim().toLowerCase()===answer.toLowerCase()){
    feedback.textContent=' ✅ Correct!';
    feedback.style.color='green';
  } else{
    feedback.textContent=` ❌ Try again! Answer: ${answer}`;
