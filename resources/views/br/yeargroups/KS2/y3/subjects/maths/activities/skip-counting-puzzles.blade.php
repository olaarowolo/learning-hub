...
<head>
  ...
  <title>Skip-counting Puzzles - Year 3 Maths</title>
...
<body>
  <header class="year-header">
    <nav aria-label="breadcrumb" class="breadcrumb-nav">
      ...
      <span>Skip-counting Puzzles</span>
    </nav>
    <h1>Skip-counting Puzzles</h1>
  ...
  <div class="container">
    ...
    <main>
      ...
      <div class="lesson-note">
        <h3>Understanding Skip-Counting Through Puzzles</h3>
        <p>Puzzles help you practice skip-counting in a fun and challenging way, improving pattern recognition and problem-solving skills.</p>
        <ul>
          <li>Complete sequences with missing numbers.</li>
          <li>Find patterns in unusual or irregular sequences.</li>
          <li>Challenge yourself to make your own puzzles for independent learning.</li>
        </ul>
        <p><strong>Tip:</strong> Write down your solutions and explain your thinking to strengthen understanding.</p>
      </div>
    </main>
    ...
    <main>
      ...
      <p>Practice skip-counting with number puzzles. Complete the missing numbers in each sequence and check your answers interactively.</p>

      <h3>Instructions</h3>
      <ul>
        <li>Observe the sequence and the step size.</li>
        <li>Fill in the missing numbers.</li>
        <li>Use logic and pattern recognition to solve the puzzles.</li>
      </ul>
    </main>
    ...
    <main>
      ...
      <div id="interactive-puzzles"></div>
    </main>
  </div>
  ...
  <script>
    const puzzleActivities = [
      ...
      {start:10, step:10, length:6},
      {start:3, step:3, length:6},
      {start:7, step:7, length:6}
    ];

    const container = document.getElementById('interactive-puzzles');

    puzzleActivities.forEach((act, idx) => {
      let seq = [];
      for(let i=0;i<act.length;i++) seq.push(act.start + i*act.step);
      const missing = [seq[1], seq[3]]; // hide 2 middle numbers for puzzle

      const div = document.createElement('div');
      div.className = 'interactive-box';
      div.innerHTML = `<strong>Puzzle ${idx+1}:</strong><br>Complete the sequence: ${seq.map((n,i)=> missing.includes(n)? `<input type="text" id="puzzle-${idx}-${i}">`: n).join(', ')}
      <button onclick="checkPuzzle(${idx})">Check Answers</button>
      <p id="puzzle-${idx}-feedback"></p>`;
      container.appendChild(div);
    });

    function checkPuzzle(idx){
      const act = puzzleActivities[idx];
      let seq = [];
      for(let i=0;i<act.length;i++) seq.push(act.start + i*act.step);
      const missing = [seq[1], seq[3]];
      let correct = true;
      missing.forEach((val)=>{
        const input = document.getElementById(`puzzle-${idx}-${seq.indexOf(val)}`);
        if(parseInt(input.value) !== val) correct = false;
      });
      const feedback = document.getElementById(`puzzle-${idx}-feedback`);
