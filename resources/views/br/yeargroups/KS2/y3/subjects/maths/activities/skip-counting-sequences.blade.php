...
<head>
  ...
  <title>Skip-counting Sequences - Year 3 Maths</title>
...
<body>
  <header class="year-header">
    <nav aria-label="breadcrumb" class="breadcrumb-nav">
      ...
      <span>Skip-counting Sequences</span>
    </nav>
    <h1>Skip-counting Sequences</h1>
  ...
  <div class="container">
    ...
    <main>
      ...
      <div class="lesson-note">
        <h3>Understanding Skip-Counting Sequences</h3>
        <p>Skip-counting sequences help you recognise patterns and relationships between numbers.</p>
        <ul>
          <li>Example: Counting by 3: 3, 6, 9, 12, 15 …</li>
          <li>Sequences can start from any number and follow a consistent step.</li>
          <li>Try making your own sequences with different step sizes to practice independent learning.</li>
        </ul>
        <p><strong>Tip:</strong> Write down sequences and check for patterns to strengthen number sense.</p>
      </div>
    </main>
    ...
    <main>
      ...
      <p>Practice completing number sequences with different step sizes. This improves recognition of arithmetic patterns and prepares you for multiplication and division.</p>

      <h3>Instructions</h3>
      <ul>
        <li>Look at the starting number and step size.</li>
        <li>Fill in the missing numbers in each sequence.</li>
        <li>Try creating your own sequences for practice.</li>
      </ul>
    </main>
    ...
    <main>
      ...
      <div id="interactive-sequences"></div>
    </main>
  </div>
  ...
  <script>
    const sequenceActivities = [
      {start:2, step:2, length:6},
      {start:5, step:5, length:6},
      {start:3, step:3, length:6},
      {start:4, step:4, length:6},
      {start:7, step:7, length:6}
    ];

    const container = document.getElementById('interactive-sequences');

    sequenceActivities.forEach((act, idx) => {
      let seq = [];
      for(let i=0;i<act.length;i++) seq.push(act.start + i*act.step);
      const missing = seq.slice(1, act.length-1); // hide middle numbers

      const div = document.createElement('div');
      div.className = 'interactive-box';
      div.innerHTML = `<strong>Sequence ${idx+1}:</strong><br>Complete the sequence: ${seq.map((n,i)=> missing.includes(n)? `<input type="text" id="seq-${idx}-${i}">`: n).join(', ')}
      <button onclick="checkSequence(${idx})">Check Answers</button>
      <p id="seq-${idx}-feedback"></p>`;
      container.appendChild(div);
    });

    function checkSequence(idx){
      const act = sequenceActivities[idx];
      let seq = [];
      for(let i=0;i<act.length;i++) seq.push(act.start + i*act.step);
      const missing = seq.slice(1, act.length-1);
      let correct = true;
      missing.forEach((val)=>{
        const input = document.getElementById(`seq-${idx}-${seq.indexOf(val)}`);
        if(parseInt(input.value) !== val) correct = false;
      });
      const feedback = document.getElementById(`seq-${idx}-feedback`);
